...
def get_usage_string(fname):
    ...
    -------
    args : list of str
        Putative usage strings for command defined by `fname`
    """
    text_list = fname.read_text().splitlines()
    usage_text = [val for val in text_list if val.lower().strip().startswith('usage')]
    if usage_text:
    	return usage_text[0]
    else:
    	return None
...
def get_help_descriptions(fname):
    """
    Parameters
    ----------
    fname : str
    ...
    helptext : str
        Full helptext of tool in `fname`
    """
    params, helptext = get_help_info(fname)
    ...
    for param in params:
        ...
        for n, f in enumerate(lines):
            f = f.strip()
            match = FINDHELP.match(f)
            if match is not None:
                f = f[match.end():]
            lines[n] = f.strip('=-\n ')
        param_descrip = '\n'.join([f for f in lines if f != ''])
        param['description'] = param_descrip.strip().capitalize()

    return params


def gen_tool_dict(help_dir, outdir='.'):
    """
    ...
    for tool in help_dir.glob('*.complete.bash'):
        ...
        params, helptext = get_help_info(get_help_fname(help_dir, tool_name),
...
def gen_args_dict(help_dir, outfile=None):
    """
    Generates dictionary of AFNI commands and arguments in `help_dir`
    ...
    outfile : str, optional
        Path to output file, if desired. Default: None

    Returns
    -------
    args_dict : dict
        Dictionary where keys are AFNI command and values are lists of args
        for command
    """
    help_dir = Path(help_dir).resolve()
...
def gen_boutiques_descriptors(help_dir, outdir='afni_descrip'):
    """
    ...
    descriptors = []
    for cmd in programs:
        params = get_help_descriptions(get_help_fname(help_dir, cmd))
